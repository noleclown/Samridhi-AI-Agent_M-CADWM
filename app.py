...
============================================
Streamlit UI matching index.html exactly — every colour, element and detail.
"""
...
# ── Logo embedded as base64 (never breaks on any host) ────────
...
try:
    ...
    st.error(f"Failed to load FAISS index: {e}"); st.stop()
...
# ══════════════════════════════════════════════════════════════
# CSS — pixel-perfect match to index.html
# ══════════════════════════════════════════════════════════════
st.markdown(f"""
<style>
...
/* ── CSS variables — exact match to index.html ───────────── */
:root {{
  --bg:           #0E1117;
  --surface:      #1A1D2E;
  --surface2:     #12141F;
...
.tts-label {{
  ...
  padding: 4px; cursor: pointer; font-size: 13px; color: var(--muted);
}}
...
/* ── Markdown in bubbles ─────────────────────────────────── */
[data-testid="stChatMessage"] h1 {{ font-size:15px; font-weight:700; margin:8px 0 4px; }}
[data-testid="stChatMessage"] h2 {{ font-size:14px; font-weight:700; margin:8px 0 4px; }}
[data-testid="stChatMessage"] h3 {{ font-size:14px; color:var(--accent); font-weight:700; margin:8px 0 4px; }}
[data-testid="stChatMessage"] p  {{ margin: 6px 0; }}
[data-testid="stChatMessage"] ul,
[data-testid="stChatMessage"] ol {{ padding-left:20px; margin:6px 0; }}
...
.thumb-btn {{
  ...
  transition: opacity 0.15s, transform 0.15s; padding: 2px 4px;
}}
.thumb-btn:hover {{ opacity:1; transform:scale(1.2); }}
...
.follow-btn {{
  ...
  transition: all 0.15s; line-height:1.4; width:100%;
}}
...
.typing-ind {{
  ...
  border-bottom-left-radius:4px; font-size:13px; color:var(--muted);
}}
...
/* ── Footer / input hint ─────────────────────────────────── */
.input-hint {{
  text-align: center; font-size: 11px; color: var(--muted); margin-top: 7px;
}}
...
with st.sidebar:
    ...
    <div class="lang-toggle">
      <button class="lang-btn {'active' if lang=='en' else ''}"
        onclick="window.parent.postMessage({{type:'streamlit:setComponentValue',value:'en'}},'*')">
        🇬🇧 EN</button>
      <button class="lang-btn {'active' if lang=='hi' else ''}"
        onclick="window.parent.postMessage({{type:'streamlit:setComponentValue',value:'hi'}},'*')">
        🇮🇳 HI</button>
    </div>""", unsafe_allow_html=True)

    # Use actual Streamlit buttons for lang switch (functional)
    ...
    # Voice toggle — matches index.html checkbox style
        ...
        if st.button("⬇ Export PDF", use_container_width=True):
            try:
                ...
                story  = [
                    ...
                    Spacer(1,12),
                    Paragraph(f"Generated: {_dt.datetime.now().strftime('%Y-%m-%d %H:%M')}", styles["Normal"]),
                    Spacer(1,12),
                ]
    ...
    # Operator panel (hidden — access via ?operator=1)
    ...
    if _params.get("operator") == "1":
        ...
        if not st.session_state.get("op_unlocked"):
            ...
            if st.button("Unlock", key="op_btn"):
                if _pwd == _op_pw:
                    st.session_state.op_unlocked = True; st.rerun()
                else:
...
# ══════════════════════════════════════════════════════════════
# CHAT HEADER — matches index.html .chat-header exactly
# ══════════════════════════════════════════════════════════════
st.markdown(f"""
...
def _follow_ups(fups, msg_idx):
    if not fups: return
    btns_html = "".join(
        f'<button class="follow-btn" onclick="window.samridhiFollowUp(this)" '
        f'data-q="{q.replace(chr(39), chr(39))}">{q}</button>'
        for q in fups
    )
    st.markdown(
        f'<div class="follow-ups">'
        f'<div class="follow-label">{ui.get("follow_label","You may also ask:")}</div>'
        f'{btns_html}</div>',
        unsafe_allow_html=True,
    )
...
if question := st.chat_input(ui["placeholder"]):
    ...
    with st.chat_message("assistant"):
        ...
        with st.spinner(ui["spinner"]):
            r = pipeline.run(question, lang, st.session_state.messages,
                             st.session_state.rate_bucket, ui)
...
# ══════════════════════════════════════════════════════════════
# FOOTER — matches index.html .input-hint exactly
# ══════════════════════════════════════════════════════════════
st.markdown("""
